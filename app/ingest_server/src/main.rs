...
struct Cli {
    ...
    /// Initial runtime IR rules path for stream correlation engine.
    #[arg(long)]
    correlation_rules_path: Option<String>,
    /// Disable near-real-time stream correlation engine.
    #[arg(long, action = ArgAction::SetTrue)]
...
struct EffectiveIngestConfig {
    ...
    correlation_rules_path: Option<String>,
    correlation_window_max_entries: usize,
    correlation_alerts_max: usize,
}
    ...
    fn from_config(cfg: &ServerConfig) -> Self {
        ...
        Self {
            ...
            ingest: EffectiveIngestConfig {
                ...
                correlation_enabled: cfg.ingest.correlation_enabled,
                correlation_rules_path: cfg.ingest.correlation_rules_path.clone(),
                correlation_window_max_entries: cfg.ingest.correlation_window_max_entries,
                correlation_alerts_max: cfg.ingest.correlation_alerts_max,
            },
        }
    }
}
...
fn apply_cli_overrides(cfg: &mut ServerConfig, cli: &Cli) {
    ...
    if let Some(v) = cli
        ...
        .filter(|v| !v.is_empty())
    {
        cfg.ingest.correlation_rules_path = Some(v.to_string());
    }
    if cli.disable_correlation {
        cfg.ingest.correlation_enabled = false;
    }
}
...
fn build_app(state: AppState, cfg: &ServerConfig) -> anyhow::Result<Router> {
    ...
    Ok(Router::new()
        ...
        .route("/api/v1/correlate/stats", get(ingest_correlate_stats))
        .route("/api/v1/correlate/alerts", get(ingest_correlate_alerts))
        .route(
            "/api/v1/correlate/rules",
            get(ingest_correlate_rules_list).post(ingest_correlate_rules),
        )
        .route("/api/v1/correlate/facts", get(ingest_correlate_facts))
        .with_state(state)
...
/// Query params for correlated alerts endpoint.
#[derive(Debug, Deserialize)]
struct CorrelateAlertsQuery {
    limit: Option<usize>,
    tenant_id: Option<String>,
...
/// Stats endpoint for central correlation engine.
async fn ingest_correlate_stats(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> ApiResult<crate::correlation::CorrelationStats> {
    let principal = state.auth.authenticate(&headers)?;
    if !matches!(principal, AuthPrincipal::Global) {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "insufficient_scope",
            "correlation stats endpoint requires a global-scope token",
        ));
    }
    Ok(Json(state.runtime.correlation.stats()))
}

/// Retrieve recent correlated alerts generated by the stream correlation engine.
async fn ingest_correlate_alerts(
    State(state): State<AppState>,
    headers: HeaderMap,
    Query(query): Query<CorrelateAlertsQuery>,
) -> ApiResult<Vec<crate::correlation::CorrelatedAlert>> {
    let principal = state.auth.authenticate(&headers)?;
    let tenant = resolve_tenant_filter(&principal, query.tenant_id.as_deref())?;
    let limit = query.limit.unwrap_or(100).clamp(1, 1_000);
    let alerts = state.runtime.correlation.alerts.recent(
        tenant.as_deref(),
...
/// Dynamic runtime IR rule registration / replacement.
async fn ingest_correlate_rules(
    State(state): State<AppState>,
    headers: HeaderMap,
    Json(body): Json<serde_json::Value>,
) -> ApiResult<LoadRulesResponse> {
    let principal = state.auth.authenticate(&headers)?;
    if !matches!(principal, AuthPrincipal::Global) {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "insufficient_scope",
            "rule registration requires a global-scope token",
        ));
    }
    let body_str = serde_json::to_string(&body).map_err(|err| {
        api_error(
            ...
            "invalid_payload",
            format!("failed to serialize body: {err}"),
        )
    })?;
    let loaded = state
        .runtime
        .correlation
        .load_from_json(&body_str)
        .map_err(|err| {
            api_error(
                StatusCode::BAD_REQUEST,
                "invalid_rule_ir",
                format!("failed to parse runtime IR rules: {err}"),
            )
        })?;
    Ok(Json(LoadRulesResponse {
        loaded,
        status: "ok",
    }))
}

/// List correlation engine rules and stats.
async fn ingest_correlate_rules_list(
    State(state): State<AppState>,
    headers: HeaderMap,
) -> ApiResult<crate::correlation::CorrelationStats> {
    let principal = state.auth.authenticate(&headers)?;
    if !matches!(principal, AuthPrincipal::Global) {
        return Err(api_error(
            StatusCode::FORBIDDEN,
            "insufficient_scope",
            "listing rules requires a global-scope token",
        ));
    }
    Ok(Json(state.runtime.correlation.stats()))
}

/// Query active facts stored in the central intel store.
async fn ingest_correlate_facts(
    State(state): State<AppState>,
    headers: HeaderMap,
    Query(query): Query<TenantQuery>,
) -> ApiResult<Vec<crate::correlation::CentralFact>> {
    let principal = state.auth.authenticate(&headers)?;
    let tenant = resolve_tenant_filter(&principal, query.tenant_id.as_deref())?;
    let facts = state.runtime.correlation.intel.list_active_facts(tenant.as_deref());
    Ok(Json(facts))
}
...
mod tests {
    ...
    #[tokio::test]
    async fn correlate_api_endpoints_load_rules_and_query_alerts_and_stats() {
        let nonce = std::time::SystemTime::now()
            .duration_since(std::time::UNIX_EPOCH)
            .expect("clock")
            .as_nanos();
        let base = std::env::temp_dir().join(format!("olopa-ingest-corr-{nonce}"));
        let mut cfg = ServerConfig::from_env();
        cfg.auth = AuthConfig::default();
        cfg.ingest.flush_workers = 1;
        cfg.ingest.wal_path = base.with_extension("wal").to_string_lossy().into_owned();
        ...
        let state = AppState {
            runtime: Arc::clone(&runtime),
            auth: Arc::new(AuthState::from_config(&cfg.auth)),
            rate_limiter: Arc::new(RateLimiter::new(100, 100, 100)),
        };
        let app = build_app(state, &cfg).expect("router");

        // 1. Post a rule into correlation engine
        let rule_payload = serde_json::json!({
            ...
            "rules": [{
                ...
                "name": "HTTP Test Rule",
                "class": "hot_path",
                "sources": [{"domain": "endpoint", "event": "process", "alias": "p"}],
                "predicates": [
                    {"op": "eq", "lhs": {"op": "field", "path": "comm"}, "rhs": {"op": "str", "value": "nc"}}
                ],
                "respond": {
                    "branches": [{
                        "actions": [{"action": "alert", "severity": "critical", "message": "Netcat detected"}]
                    }]
                }
            }]
        });
        ...
        let post_rules = app
            .clone()
            .oneshot(
                axum::http::Request::post("/api/v1/correlate/rules")
                    .header(header::CONTENT_TYPE, "application/json")
            ...
            .await
            .unwrap();
        assert_eq!(post_rules.status(), StatusCode::OK);

        // 2. Query correlate stats
        let stats_resp = app
            .clone()
            .oneshot(
                axum::http::Request::get("/api/v1/correlate/stats")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
        ...
        // 3. Send an ingest batch that triggers the rule
        ...
        let alerts_resp = app
            .clone()
            .oneshot(
                axum::http::Request::get("/api/v1/correlate/alerts?tenant_id=test-tenant")
                    .body(Body::empty())
                    .unwrap(),
            )
            .await
