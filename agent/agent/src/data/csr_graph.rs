...
impl CsrSnapshot {
    ...
    pub fn find_edge(&self, src: u32, dst: u32) -> Option<&EdgeProps> {
        ...
        neighbors.iter().position(|&n| n == dst).map(|i| &props[i])
    }
    ...
    pub fn k_hop_subgraph(&self, trigger: u32, k: u8) -> (Vec<u32>, Vec<(u32, u32, EdgeProps)>) {
        let reachable = self.bfs(trigger, k);
        let node_set: std::collections::HashSet<u32> = reachable.iter().map(|(v, _)| *v).collect();
