...
fn copy_prebuilt_ebpf(out_dir: &Path) -> Result<bool, Box<dyn std::error::Error>> {
    ...
    let explicit = env::var("OLOPA_EBPF_PREBUILT").ok().map(PathBuf::from);
    ...
    for candidate in explicit.into_iter().chain([default_prebuilt]) {
        if candidate.is_file() {
            ...
            println!(
                "cargo:warning=using prebuilt eBPF object: {}",
                candidate.display()
            );
            return Ok(true);
        }
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let out_dir = env::var_os("OUT_DIR").ok_or_else(|| io::Error::other("OUT_DIR not set"))?;
